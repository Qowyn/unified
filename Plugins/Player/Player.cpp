                    ...
                    if (auto *pObject = Utils::AsNWSObject(Utils::GetGameObject(oidObjectToUpdate)))
                    {
                        ...
                        if (type == Services::Hooks::CallType::BEFORE_ORIGINAL)
                        {
                            if (auto loopingVisualEffectSet = g_plugin->GetServices()->m_perObjectStorage->Get<void*>(oidObjectToUpdate,
                                    "LVES!" + Utils::ObjectIDToString(pPlayer->m_oidPCObject)))
                            {
    ...
    if (auto *pPlayer = player(args))
    {
        ...
        if (visualEffect < 0)
        {
                    ...
                    "LVES!" + Utils::ObjectIDToString(pPlayer->m_oidPCObject)))
            {
                ...
                g_plugin->GetServices()->m_perObjectStorage->Remove(oidTarget, "LVES!" + Utils::ObjectIDToString(pPlayer->m_oidPCObject));
        ...
        else
        {
            ...
            else
            {
                ...
                g_plugin->GetServices()->m_perObjectStorage->Set(oidTarget, "LVES!" + Utils::ObjectIDToString(pPlayer->m_oidPCObject),
